...
The HTML report generated by infiltr provides an assessment dashboard including severity breakdowns across all findings, an interactive MITRE ATLAS heatmap — every bundled technique laid out by tactic and colored by the highest-severity finding, where selecting a technique filters the findings to it — and per-finding ATLAS technique and tactic, attack prompt, target response, and remediation guidance for validation and triage. Additional JSON and SARIF formats (stamped with the ATLAS release, with SARIF rules linking to each technique on atlas.mitre.org) enable seamless integration into CI/CD pipelines and GitHub Security workflows.
