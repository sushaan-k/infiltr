...
infiltr covers 24 MITRE ATLAS techniques across 6 tactics, with full coverage of prompt injection (AML.T0051), goal hijacking (AML.T0054), and data exfiltration (AML.T0024) categories. Each finding is automatically mapped to the specific technique exploited, enabling direct remediation guidance and detailed compliance reporting.

## Report Showcase

The HTML report generated by infiltr provides a comprehensive assessment dashboard including severity breakdowns across all findings, an interactive ATLAS heatmap showing which techniques were successfully exploited, and per-finding reproduction steps for validation and remediation. Additional JSON and SARIF formats enable seamless integration into CI/CD pipelines and GitHub Security workflows.
